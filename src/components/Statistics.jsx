...
export default function Statistics() {
  ...
  const patientIllness = {Cancer: 30, Diabetes: 45, Hipertension: 25, Esquizofrenia: 12}
  const totalPatients = Object.values(patientIllness).reduce((acc, value) => acc + value, 0);

  const donors = {ONG: 3, Gurbernamental: 2, Privada: 2, Natural: 2};
  const totalDonors = Object.values(donors).reduce((acc, value) => acc + value, 0);

  const donorsDonations = {Caritas: 120, AliVen: 53, MMPS: 70, SPPS: 30, ManosUnidas: 40, Farmatodo: 45, Central: 20, Jose: 10, Maria: 8  };
          ...
          <div className={'statistics-second-row'}>
            <div className={'card donors'}>
              ...
              <BarChart
                  ...
                  yAxis={[{scaleType: 'band', data: sortedDonors.map(item => item.name)}]}
                  height={320}
                  width={560}
                  layout={'horizontal'}
