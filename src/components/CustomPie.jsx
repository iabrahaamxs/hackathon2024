...
const CustomPie = ({ data, title, color, size }) => {
  ...
  return(
      <div className={'custom-pie'} style={{position: 'relative', width: {size}, height: {size}}}>
        <PieChart
            series={[
              {
                data: coloredData,
        ...
        <div style={{textAlign: 'center', marginTop: '-40px', marginLeft: '-100px', fontSize: '15px', fontWeight: 'bold'}}>
          {title}
        </div>
      </div>
  );
